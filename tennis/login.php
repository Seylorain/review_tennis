...
Решение проблемы:	Ограничить количество попыток.
			...
			if(empty($_SESSION['ban']['time']))
			{ 
				$_SESSION['ban']['time'] = $time; 
			}
			if(empty($_SESSION['ban']['rate']))
			{ 
				$_SESSION['ban']['rate'] = 1; 
			}
			if($_SESSION['ban']['time']+$max_t < $time){
  				unset($_SESSION['ban']);
  				auth_function(); //Здесь выполняется код на проверку логина и пароля
			}
			else{
...
Решение проблемы 	Нельзя брать данные напрямую в запросе, необходима их проверка на валидность. Можно воспользоваться  функцией
			mysql_real_escape_string(). 
			Данная функция требует установить соединение с БД, перед использованием.
